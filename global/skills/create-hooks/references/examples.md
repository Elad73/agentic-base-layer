...
Notification commands are platform-specific. Use the variant that matches your OS (`notify-send` on Linux, `osascript`/`terminal-notifier` on macOS, PowerShell toast on Windows).
