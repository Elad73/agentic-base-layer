...
<common_patterns>
**Desktop notification when input needed** (use a notifier for your platform — `notify-send` on Linux, `osascript`/`terminal-notifier` on macOS, `msg`/PowerShell on Windows):
```json
{
  "hooks": {
    "Notification": [
