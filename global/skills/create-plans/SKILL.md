...
<domain_expertise>
**Optional: load a domain-expertise skill if one is installed.**

If you have a separate skill that bundles deep, stack-specific knowledge (conventions, pitfalls, recommended architecture for a particular kind of project), load it before creating a roadmap or phase plan. This is a *soft* dependency — if no such skill is installed, proceed without it. Nothing here requires a domain skill to exist.

<scan_domains>
Check whether any domain-expertise skill is available:
```bash
ls ~/.claude/skills/ 2>/dev/null
```

**If no matching domain skill is found:** proceed without domain expertise (graceful degradation). This skill works fine without domain-specific context.
</scan_domains>

<inference_rules>
If the user's request points at a specific stack (a native mobile/desktop app, a game engine, an Agent SDK app, a Python automation, a frontend, etc.) and a matching domain-expertise skill happens to be installed, infer it and confirm:
```
Detected: [domain] project → matching domain skill: [skill-name]
Load this expertise for planning? (Y / proceed without it)
```
</inference_rules>

<load_domain>
If a domain-expertise skill is installed and selected, load it intelligently:

**Step 1: Read the domain SKILL.md**
```bash
cat ~/.claude/skills/[domain-skill]/SKILL.md 2>/dev/null
```
...
A well-built domain skill contains a `<references_index>` section that maps planning contexts to specific references.

Example:
```markdown
<references_index>
**For database/persistence phases:** references/persistence.md, references/concurrency.md
**For UI/layout phases:** references/ui-layout.md, references/design-guidelines.md
**For system integration:** references/platform-integration.md
**Always useful:** references/conventions.md
</references_index>
```
...
```bash
# Example: planning a persistence phase
cat ~/.claude/skills/[domain-skill]/references/persistence.md
cat ~/.claude/skills/[domain-skill]/references/conventions.md
```
...
**If no domain skill is installed:** proceed without domain expertise — do not block.

**If the domain SKILL.md has no references_index:** fall back to loading all its references, with a note about context usage.
</load_domain>
