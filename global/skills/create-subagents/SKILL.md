...
<subagents_vs_skills>
## Subagents vs. Skills — pick the right tool

| | **Subagent** | **Skill** |
|---|---|---|
| **What it is** | A *separate* Claude instance with its own isolated context window | Reusable knowledge/procedure loaded into the *current* context |
| **Context** | Isolated; only its final report returns to the main thread | Shares your context; everything it loads stays in your window |
| **Best for** | Heavy fan-out/parallel work, noisy exploration, role-isolated tasks (review, research) | Encoding how-to knowledge, conventions, multi-step procedures you run inline |
| **Tools** | Can be restricted to a focused, least-privilege tool set | Uses whatever tools the session already has |
| **Invocation** | Launched explicitly via the Task tool (or auto-selected by `description`) | Auto-triggered by its `description`, or via the Skill tool |
| **State** | Returns a summary; intermediate work is discarded | Edits files, accrues context you keep |

Rule of thumb: reach for a **subagent** when you want to *offload* a chunk of work into a clean context; reach for a **skill** when you want Claude to *know how* to do something in-line (see the create-agent-skills skill).
</subagents_vs_skills>

<first_invocation_rule>
## The first invocation must carry everything

A subagent cannot ask you a clarifying question mid-run — it is a black box that runs to completion and returns only its final output. There is no back-and-forth.

Therefore the prompt you pass on the **first (and only) invocation must be self-contained**:
- All requirements, constraints, and acceptance criteria stated up front
- Any file paths, identifiers, or values the agent needs (it cannot guess your context)
- The exact shape of the deliverable you want back
- What it must NOT do

If the task is genuinely ambiguous, resolve the ambiguity in the **main chat first** (using AskUserQuestion), then launch the subagent with the resolved decision baked in. Never delegate a half-specified task and expect the agent to come back for clarification — it can't.
</first_invocation_rule>
...
- Model selection (Sonnet + Haiku orchestration)
