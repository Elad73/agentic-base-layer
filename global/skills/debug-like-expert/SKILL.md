...
# Scan for any installed domain-expertise skills (optional)
echo "DOMAIN_SKILLS:"
...
If you have a separate skill that bundles deep, domain-specific knowledge (debugging, testing, performance, common pitfalls for a particular stack), load it before investigation. This is a *soft* dependency — if no such skill is installed, proceed with the general debugging methodology below. Nothing here requires a domain skill to exist.
...
**If no matching domain skill is found:** proceed without it (graceful degradation). This skill works fine with general debugging methodology alone.
</scan_domains>

<inference_rules>
If the user's description or codebase points at a specific stack (e.g. a Python game, a Rust systems program, a Swift app, a Next.js frontend, an Agent SDK project), and a matching domain-expertise skill happens to be installed, infer it and confirm:
```
Detected: [domain] issue → matching domain skill: [skill-name]
Load this debugging expertise? (Y / proceed with general methodology)
```
</inference_rules>

<load_domain>
If a domain-expertise skill is installed and selected, read its references before investigating:
```bash
cat ~/.claude/skills/[domain-skill]/references/*.md 2>/dev/null
```
...
**If no domain skill is installed:** proceed with the general methodology — do not block.
</load_domain>

<when_to_load>
Load domain expertise BEFORE investigation when the domain is known and a matching skill exists.

Domain expertise is NOT needed for:
- Pure logic bugs (domain-agnostic)
- Generic algorithm issues
- When the user explicitly says "skip domain context"
</when_to_load>
</domain_expertise>
